...
// The signup CTA rendered in place of the download/preview surface for
// anon users. Same Google G we use on the home hero, wrapped in a
// deep-link so on return the client can bind the freshly-completed
// jobId to the new account (see PostSignInClaimer).
function GoogleSignInGate({
  ...
  const returnPath = audioType === "music" ? "/audio-mastering" : "/";
  const claimQuery = jobId ? `?claim=${encodeURIComponent(jobId)}` : "";
  const returnTo = encodeURIComponent(`${returnPath}${claimQuery}`);
  const signUpHref = `/handler/sign-up?after_auth_return_to=${returnTo}`;
  const signInHref = `/handler/sign-in?after_auth_return_to=${returnTo}`;

  return (
    <div className="mt-6 rounded-2xl border border-(--border-medium) bg-(--bg-elevated) p-6 text-center">
      <p className="text-base font-semibold mb-1">{heading}</p>
      <p className="text-xs text-(--text-muted) mb-5">{subheading}</p>

      <Link
        href={signUpHref}
        className="inline-flex items-center justify-center gap-3 w-full max-w-sm mx-auto px-5 py-3 rounded-full bg-white text-[#1f1f1f]! font-medium shadow-sm hover:shadow-md transition-shadow"
      >
      ...
      <p className="mt-4 text-xs text-(--text-muted)">
        ...
        <Link href={signInHref} className="underline text-(--text-secondary) hover:text-(--text-primary)">
          Sign in
        </Link>
        {" "}· Prefer email or GitHub? Use the same button — all options are on the sign-in page.
      </p>
    </div>
  );
}
...
export default function MasteringTool({
  ...
  // Retained: fires the Google Ads conversion event exactly once per
  // completed master. The anon email-notification flow that used to live
  // here was removed — it was the leak (an anon user got the raw download
  // URL mailed to them and never had to sign up), which is why signup
  // rate collapsed. Sign-in is now the only path to the file.
  ...
  const startMastering = async () => {
    ...
    try {
      ...
      // Auto-subscribe signed-in users for completion email. The emailed
      // link routes through /api/mastering/download so it still forces
      // a live Stack Auth session before serving the file.
    ...
    <div className={compact ? "" : ""}>
      ...
      {!isProcessing && !isCompleted && !isFailed ? (
        ...
        <div className="glass-card p-8 text-center">
          ...
          {/* Post-processing gate: signed-in users get a soft "we've got it"
              affordance; anon users get a hard sign-in CTA so the download
              gate at the completion moment doesn't feel like a bait-and-switch. */}
          {user ? (
            <div className="mt-6 p-4 rounded-lg bg-(--bg-secondary) border border-(--border-subtle)">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-(--success)" />
                <p className="text-sm">
                  You can close this tab safely — your master will be waiting
                  in <Link href="/dashboard/files" className="underline">your dashboard</Link>.
                </p>
              </div>
            </div>
          ) : (
            <GoogleSignInGate
              jobId={jobId}
              audioType={audioType}
              heading="Sign in now — we'll deliver your master the moment it's ready"
              subheading="Free · one click with Google · your file is saved to your account"
            />
          )}
        </div>
      ) : isCompleted ? (
        ...
        <div className="glass-card p-8 text-center">
          ...
          <p className="text-(--text-secondary) text-sm mb-4">
            {user ? labels.readyMessage : "Sign in with Google to hear it and grab your download."}
          </p>
          ...
          {user && !isSubscribed && (
            <p className="text-xs text-(--text-muted) mb-4">
              Saved to your account · <Link href="/pricing" className="text-(--warning) hover:underline">Upgrade</Link> to keep more than one master at a time.
            </p>
          )}

          {/* Preview + download surface for signed-in users only. We never
              render the raw Modal URL for anon users — that's exactly what
              was letting people grab the file without an account. */}
          {user && jobId && (
            <div className="mb-6">
              <AudioPlayer src={`/api/mastering/download/${jobId}`} />
            </div>
          )}
          ...
          {user ? (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <button
                  onClick={() => {
                    const audio = new Audio(`/api/mastering/download/${jobId}`);
            ...
            <>
              <GoogleSignInGate
                jobId={jobId}
                audioType={audioType}
                heading="Sign in to hear + download your master"
                subheading="Free forever · one click with Google · your file is saved to your account"
              />
              <button
                onClick={resetSession}
                className="mt-3 text-sm text-(--text-muted) hover:text-(--text-primary) inline-flex items-center gap-1.5"
              >
                <RefreshCw className="w-4 h-4" />
                Master a different file instead
              </button>
            </>
          )}
        </div>
      ...
      <AnimatePresence>
        {showVideoGenerator && jobId && (
          <VideoGenerator
            audioUrl={`/api/mastering/download/${jobId}`}
